...
if os.path.exists("urls_serial.txt"):
  os.remove("urls_serial.txt")
if os.path.exists("inverted_index_serial.json"):
  os.remove("inverted_index_serial.json")
...
with open("urls_serial.txt", mode="w") as file:
    file.write("\n".join(urls_crawled))

try:
    with open("inverted_index_serial.json", mode="r") as file:
        json_data = json.load(file)
        index = json_data["index_list"]
except IOError:
    with open("inverted_index_serial.json", mode="w") as file:
        new_data = {"index_list": {}}
